...
def main():
    ...
    with torch.no_grad():
        ...
        last_hidden_state = model(**batch_tokens,
    ...
    r = r.unsqueeze(0)
    r = r.unsqueeze(-1)
    weights = (r.expand(last_hidden_state.size()).float().to(
        last_hidden_state.device))

    # Get attn mask of shape [bs, seq_len, hid_dim]
    input_mask_expanded = (batch_tokens["attention_mask"].unsqueeze(-1).expand(
