...
class Email:
    ...
    def __init__(self, thread_id: str, email_id: str, unread: bool,
                 message_raw):
        self.thread_id = thread_id
        self.email_id = email_id
        self.unread = unread
        email_message = email.message_from_bytes(
            base64.urlsafe_b64decode(message_raw["raw"]))
        ...
        if email_message.is_multipart():
            ...
            self.body += (text_body
                          if text_body else html_body).decode("utf-8")
        else:
...
class GmailHelper:
    ...
    def get_email_threads(self) -> List[EmailThread]:
        ...
        for thread_id in thread_ids:
            ...
            threads = self.service.users().threads().get(
            ...
            for message in threads.get("messages", []):
                ...
                email_id = message["id"]
                message_raw = self.service.users().messages().get(
                    userId='me', id=email_id, format="raw").execute()
                email_message = Email(thread_id, email_id, email_id in unread,
                                      message_raw)
